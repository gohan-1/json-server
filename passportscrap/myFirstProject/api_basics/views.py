...
def convert(request):
    if request.method == 'POST':
        json_data = json.loads(request.body)
        f_type = json_data['f_type']
        if "Excel" == f_type:
            file = json_data['myfile']
            print(file)
            buf = io.BytesIO()
            ...
            with open(file, encoding="utf8") as f:
                ...
                excel_writer.save() 
                buf.seek(0)
            response = HttpResponse(FileWrapper(buf), content_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheetmsword')
            ...
            with open(file, encoding="utf8") as f:
                ...
                mem.seek(0)
                buf.close()
                response = HttpResponse(FileWrapper(mem), content_type='application/msword')
            ...
            with open(file, encoding="utf8") as f:
                ...
                dom = parseString(xml)
                op = dom.toprettyxml()     
                response = HttpResponse(op, content_type='application/xml')
                response['Content-Disposition'] = 'attachment; filename="%s.xml"',file
                return response

    return HttpResponse("outside excel")
